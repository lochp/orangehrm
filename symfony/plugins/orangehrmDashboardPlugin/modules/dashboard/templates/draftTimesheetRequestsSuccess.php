            ...
            if ($recCount > 0):
                ...
                foreach ($timesheetList as $key => $timesheetData):
                    ...
                    <tr class="<?php echo ($count & 1) ? 'even' : 'odd' ?>">
                        <td>
                            <a href="<?php echo url_for('time/viewEmployeeTimesheet') . '/id/' . $timesheetData['timesheetId'] ?>">
                                <?php
                                $count++;
                                echo $timesheetData['firstName'] . " " . $timesheetData['lastName'] . " " . "    " . $timesheetData['date'];
                                ?>
                            </a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr class="odd"><td><?php echo __("No Draft Timesheet"); ?></td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
        ...
        <table class="table">
            <tr class="total">
                <td style="text-align:left;padding-left:20px; cursor: pointer">
                    <?php
                    $date = draftTimesheetRequestsAction::getDateRange();
