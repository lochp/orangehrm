...
class AddEmployeeForm extends sfForm {
    ...
    public function configure() {
        ...
        //Marc Modif
        //When we transfer the database, need to change the nextId
        ...
        if ($employeeService->getEmployee(intval($empNumber))){
            $idGenService->updateEmpNumber();
        }
    ...
    protected function getFormLabels() {
        $labels = array(
        	...
        	'country' => __('Working Country') . '<em> *</em>',
    ...
    public function getEmployee(){
        ...
        try{
        	if (!is_null($posts['country'])){
        		$searchParams = array('cou_code' => $posts['country']);
        		$list = $this->getCountryService()->searchCountries($searchParams);
        		if (!is_null($list) && !is_null($list->getFirst())){
        			$country = $list->getFirst();
