...
class EmployeeDao extends BaseDao {
    ...
    protected static $searchMapping = array(
        ...
        //Marc modif
            'operational_country' => 'e.coun_code'
    );
    ...
    /**
     * Retrieve employeeOperationalCountry by empNumber, must make this retrieve domain object
     * @param int $empNumber
     * @returns boolean
     * @throws DaoException
     */
    public function getEmployeeOperationalCountry($empNumber) {
        try {
            $q = Doctrine_Query:: create()->from('OperationalCountry oc')
                ->where('oc.emp_number = ?', $empNumber)
                ->orderBy('oc.name ASC');
            return $q->execute();
        } catch (Exception $e) {
            throw new DaoException($e->getMessage(), $e->getCode(), $e);
