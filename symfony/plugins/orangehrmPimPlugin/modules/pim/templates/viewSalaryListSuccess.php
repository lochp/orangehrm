...
<div class="box pimPane">
    ...
    <?php if ($salaryPermissions->canCreate() || (count($salaryList) > 0 && $salaryPermissions->canUpdate()) || $sf_user->getAttribute('auth.isAdmin') == "Yes") { ?>
    <div id="changeSalary">
        <div class="head">
                ...
                <fieldset>
                    ...
                    <p>
                        ...
                        <?php if (
                                ...
                                (count($salaryList) > 0 && $salaryPermissions->canUpdate()) || $sf_user->getAttribute('auth.isAdmin') == "Yes"
                                ) { ?>
    ...
    <div class="miniList" id="salaryMiniList">
        ...
        <div class="inner">
            ...
            <form id="frmDelSalary" action="<?php echo url_for('pim/deleteSalary?empNumber=' . $empNumber); ?>" method="post" class="longLabels">
                <?php echo $listForm ?>
                <p id="actionSalary">
                    <?php if ($salaryPermissions->canCreate()  || $sf_user->getAttribute('auth.isAdmin') == "Yes") { ?>
                    <input type="button" value="<?php echo __("Add"); ?>" class="" id="addSalary" />
                    <?php } ?>
                    <?php if (($salaryPermissions->canDelete() && count($salaryList) > 0) || ($sf_user->getAttribute('auth.isAdmin') == "Yes" && count($salaryList) > 0)) { ?>
                    ...
                    <tbody>
                        ...
                        foreach ($salaryList as $salary) :
                            ...
                            <tr class="<?php echo $cssClass; ?>">
                                ...
                                <?php } ?>
                                <td class="component">
                                    <input type="hidden" id="code_<?php echo $salary->id; ?>" value="<?php echo $salary->id; ?>" />                                      <?php if($salaryPermissions->canUpdate()  || $sf_user->getAttribute('auth.isAdmin') == "Yes") {?>
...
//<![CDATA[
    ...
    $(document).ready(function() {
		<?php
		if ($sf_user->getAttribute('auth.isAdmin') == "Yes"){
		?>
		$("#frmSalary input,select,textarea,checkbox").each(function(){
			if($(this).is(':disabled'))
