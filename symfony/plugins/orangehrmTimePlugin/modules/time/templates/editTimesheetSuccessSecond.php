        ...
        <form class="timesheetForm" method="post" id="timesheetForm" >
            ...
            <div class="tableWrapper" style="overflow:auto">
                <table style="width:100%" class="table">
                        ...
                        <?php foreach ($currentWeekDates as $date): ?>
                            <?php if ((substr(date('D', strtotime($date)), 0, 1) == "S") || $holidayService->isHoliday($date,$operationalCountry)): ?>
                                <th class="center weekend" style="width:2%">
                                    <?php echo __(substr(date('D', strtotime($date)),0,1) . "<br>"); ?>
                    ...
                    <tbody>
                        <?php $x=0; echo $timesheetForm['_csrf_token']->render(); ?>
                        <?php $numOfRows = sizeof($timesheetForm['initialRows']);
                            for ($i = 0 ; $i <$numOfRows ; $i++) {
                                ?>
                                <tr class="<?php echo ($x & 1) ? 'even' : 'odd' ?>">
                                    <td id="<?php echo "projectId" . "_" . "activityId" . "_" . $timesheetId . "_" . $employeeId ?>">
                                        ...
                                        <?php if (!$test) : ?>
                                            <td class="center comments">
                                                ...
                                                <?php if ((substr(date('D', strtotime($currentWeekDates[$j])), 0, 1) == "S") || $holidayService->isHoliday($currentWeekDates[$j],$operationalCountry)): ?>
                                                    <?php
                                                    if (!$authorizeClick || $authorizeClick == "Yes"):
                                                        echo $timesheetForm['initialRows'][$i][$j]->render(array("class" => " " . 'timeBox weekend', "onclick" => " " . 'javascript:onClickTimeBox(this)')) ;
                                                    else:
                                                        echo $timesheetForm['initialRows'][$i][$j]->render(array("class" => " " . 'timeBox weekend'));
                                                    endif;
                                                        ?>
                                                ...
                                                <?php else:
                                                    if (!$authorizeClick || $authorizeClick == "Yes"):
                                                        echo $timesheetForm['initialRows'][$i][$j]->render(array("class" => " " . 'timeBox week', "onclick" => " " . 'javascript:onClickTimeBox(this)'));
                                                    else :
                                                        echo $timesheetForm['initialRows'][$i][$j]->render(array("class" => " " . 'timeBox week'));
                                                    endif;
                                                    ?>
                                                    <!--                                        <span class="" data-toggle="modal" href="#commentDialog">-->
                                                    <?php echo image_tag(theme_path('images/comment.png'), 'id=commentBtn_' . $j . '_' . $i . " class=commentIcon " ) ?>
            ...
            <!-- Il faut faire un js qui affiche la page comme _addRow -->
            <script>
                function addAlreadyRow(num, startDate, endDate, employeeId, timesheetId) {
                    var r = $.ajax({
                        type: 'GET',
                        url: linkAlreadyRow ,
                        data: "num="+num+"&startDate="+startDate+"&endDate="+endDate+"&employeeId="+employeeId+"&timesheetId="+timesheetId+"&values="valuesJSON,
                        async: false
                    }).responseText;
                    return r;
                }
            ...
            <script>
                $("#extraRows").append(addAlreadyRow(rows-1, startDate, endDate, employeeId, timesheetId));
...
<script type="text/javascript">
    ...
    var linkAlreadyRow = "<?php echo url_for('time/addAlreadyRow') ?>";
