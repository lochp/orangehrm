...
class editTimesheetAction extends baseTimeAction {
    ...
    public function execute($request) {
        ...
        //See if the user has the possibility to use the click on timesheetItems
        if ($loggedInEmpNumber != 0) {
            $employeeService = new EmployeeService();
            $employee = $employeeService->getEmployee($loggedInEmpNumber);
            $this->authorizeClick = $employee->getCustom2();
        }
        ...
        //Marc Modif
        //Add isduplicate for the timesheetForm
        $values = array('date' => $this->startDate, 'employeeId' => $this->employeeId, 'timesheetId' => $this->timesheetId, 'noOfDays' => $this->noOfDays, 'isduplicate' => $this->isduplicate);
    ...
    public function getPreviousTimesheet($startDate,$employeeId) {
        $timesheets = $this->getTimesheetService()->getTimesheetByEmployeeId($employeeId);
        foreach($timesheets as $timesheet){
            if ($timesheet->getStartDate() == $startDate) {
                return $timesheet;
            }
        }
        return null;
