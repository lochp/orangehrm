...
class TimesheetService {
            ...
            if ($activityId != null) {
                ...
                for ($i = 0; $i < sizeof($keysArray); $i++) {
                    ...
                    if ($timesheetItemId != null) {
                        //Ici, l'item existe deja, il s'agit donc de regarder si la nouvelle valeur est 0 ou non. Si 0, on supprime l'item, sinon on laisse ce qui est fait
                        ...
                        if ($timesheetItemDuration == 0){

                            $this->getTimesheetDao()->deleteTimesheetItem($timesheetItemId);
                        ...
                        //Ici nouvel item avec une duration non nulle, donc on fait rien
                        ...
                        if ($existingTimesheetItem[0]->getProjectId() != null) {
                            ...
                            //Ici, on a un nouveau item, mais sans duration = null du coup pas de save ?
        ...
        if ($timesheet == null) {
            ...
            if ($this->checkForOverlappingTimesheets($timesheetStartingDate, $endDate, $employeeId) == 0) {
                if ($this->checkForMatchingTimesheetForCurrentDate($employeeId, date('Y-m-d')) == null) {
                    //state 1 is given when timesheet is overlapping + matching timesheet is not found
    ...
    public function getProjectNameByProjectId($projectId) {
        ...
        return $this->getTimesheetDao()->getProjectNameByProjectId($projectId);
    }

    public function getTimesheetId($startDate,$employeeId){
        $timesheets = $this->getTimesheetDao()->getTimesheetByEmployeeId($employeeId);
    ...
    public function getTimesheetByEndDateAndEmployeeId($endDate, $employeeId) {

        $timesheet = $this->getTimesheetDao()->getTimesheetByEndDateAndEmployeeId($endDate, $employeeId);
