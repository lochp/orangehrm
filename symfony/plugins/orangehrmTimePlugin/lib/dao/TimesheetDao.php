...
class TimesheetDao {
    ...
    //Marc Modif Timesheet delete timesheet when trying to save the timesheet
    public function deleteTimesheetItem($timesheetItemId){
        try {

            $query = Doctrine_Query::create()
                ->delete()
                ->from("TimesheetItem")
                ->where("timesheetItemId = ?", $timesheetItemId);
    ...
    public function getTimesheetByEmployeeIdAndStates($employeeId, $state) {
    ...
    public function getProjectAndActivities($timesheetId){

        try {
            $query = Doctrine_Query::create()
                ->from("TimesheetItem")
                ->where("timesheet_id = ?", $timesheetId);
            $record = $query->execute(array(), Doctrine::HYDRATE_SCALAR);
        } catch (Exception $ex) {
            throw new DaoException($ex->getMessage());
        }
