...
//             	$objDrawing->setWorksheet($objPHPExcel1->setActiveSheetIndex(0));
            	 ...
            	 * Code Logo
            	 */
            	$gdImage = imagecreatefromjpeg('images/upskills.jpg');
            	// Add a drawing to the worksheetecho date('H:i:s') . " Add a drawing to the worksheet\n";
            	$objDrawing = new PHPExcel_Worksheet_MemoryDrawing();
            	$objDrawing->setName('Logo');
            	$objDrawing->setDescription('Upskills Logo');
            	$objDrawing->setImageResource($gdImage);
            	$objDrawing->setRenderingFunction(PHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG);
            	$objDrawing->setMimeType(PHPExcel_Worksheet_MemoryDrawing::MIMETYPE_DEFAULT);
            	$objDrawing->setHeight(83);
...
//             							$sumDays += round($inHour/8, 1);
            							$dayValue = $inHour <= 2? 0: $inHour <= 6? 0.5: 1;
            							$sumDays += $dayValue;
            							$sumHours += $inHour;
...
//             					$index++;
                ...
                // Modif XGL to print on 1 sheet by default
                $objPHPExcel1->getActiveSheet()->getPageSetup()->setPaperSize(PHPExcel_Worksheet_PageSetup::PAPERSIZE_A4);
                $objPHPExcel1->getActiveSheet()->getPageSetup()->setFitToPage(true);
                $objPHPExcel1->getActiveSheet()->getPageSetup()->setFitToWidth(1);
                $objPHPExcel1->getActiveSheet()->getPageSetup()->setFitToHeight(1);
...
//            header('Content-Disposition: attachment;filename="' . $this->reportName . '.xls"');
            header('Content-Disposition: attachment;filename="' . 'Timesheet_' . $employee->getLastName() . '_' . $employee->getFirstName() . '_' . $projectNameWithCustomerName . '_' . date('d-M-Y', $fromDate) . '_to_' . date('d-M-Y', $toDate) .'.xlsx"');
            header('Cache-Control: max-age=0');
...
//             $objWriter->save("TEMP.xlsx");
        ...
        if ($reportId == 1){
            ...
            $objPHPExcel1->getProperties()->setCreator("Upskills - Reports")
                ...
                ->setKeywords("upskills excel timesheet")
                ->setCategory("Timesheet");
