...
class viewLeaveBalanceReportAction extends sfAction {
    ...
    public function execute($request) {
        ...
        if ($runReport) {
            ...
            //todo : get the leaves for all employees in case of reportId = 1

            if ($request->getParameter('export')){
                if ($reportId == 1) {

                    $leaveTypeDao = new LeaveTypeDao();
                    $employeeId = $values['empNumber'];

                    //Get the from and to Dates
                    if ($values["fromDate"]) {
                        $fromDate = strtotime($values["fromDate"]);
                    } else {
                        $fromDate = strtotime("1970-01-01");
                    }
                    if ($values["toDate"]) {
                        $toDate = strtotime($values["toDate"]);
                    } else {
                    ...
                    if ($employeeId) {
                        ...
                        $objPHPExcel1->getProperties()->setCreator("Upskills - Reports")
                            ...
                            ->setTitle("Upskills - Reports on leaves")
                            ->setSubject("Upskills - Reports on leaves")
                        ...
                        $column++;
                        $objPHPExcel1->setActiveSheetIndex(0)
                            ->setCellValue($column . $row, "Leave Type");
                        $column++;
                        $objPHPExcel1->setActiveSheetIndex(0)
                        ...
                        $column++;
                        $objPHPExcel1->setActiveSheetIndex(0)
                            ->setCellValue($column . $row, "Leave Status");
                        $column++;
                        $objPHPExcel1->setActiveSheetIndex(0)
                        ...
                        $leaveEntitlementDao = new LeaveEntitlementDao();
                        $leaves = $dao->getLeaves($employeeId);

                        foreach ($leaves as $leave) {
                            $date = strtotime($leave['date']);
                            if (($fromDate <= $date && $date <= $toDate)) {

                                $leaveType = $leaveTypeDao->readLeaveType($leave['leave_type_id']);
                                $leaveTypeName = $leaveType['name'];
                                $leaveStatus = $leaveEntitlementDao->getLeaveStatusName($leave['status']);
                                ...
                                $column++;
                                $objPHPExcel1->setActiveSheetIndex(0)
                                    ->setCellValue($column . $row, $leaveTypeName);
                                $column++;
                                $objPHPExcel1->setActiveSheetIndex(0)
                                    ->setCellValue($column . $row, date('Y-m-d', $date));
                                $column++;
                                $objPHPExcel1->setActiveSheetIndex(0)
                                    ->setCellValue($column . $row, $leave['length_hours']);
                                $column++;
                                $objPHPExcel1->setActiveSheetIndex(0)
                                    ->setCellValue($column . $row, $leaveStatus[0]["name"]);
                                $column++;
                                $objPHPExcel1->setActiveSheetIndex(0)
                                    ->setCellValue($column . $row, $leave['comments']);
                                $row++;
                            }
                        }
                ...
                if ($reportId == 2) {
                    ...
                    if ($empNumbers) {
                        ...
                        foreach ($empNumbers as $empNumber) {
                            ...
                            $leaveEntitlementDao = new LeaveEntitlementDao();
                            $dao = new LeaveRequestDao();
                            $leaves = $dao->getLeaves($empNumber);
                            ...
                            foreach ($leaves as $leave) {
                                ...
                                if (($fromDate <= $date && $date <= $toDate)) {
                                    if ($leaveTypeId == 0 || $leaveTypeId == $leave['leave_type_id']) {
