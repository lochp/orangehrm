...
//        $leaveRequest->setLeavePeriodId($leavePeriodId);
    ...
    protected function isHoliday($day, LeaveParameterObject $leaveAssignmentData) {
        ...
        $workSchedule = $this->getWorkScheduleService()->getWorkSchedule($empNumber);       
        /**
         * Loc add country of employee for isHoliday and Marc
         * @var unknown
         */
        $employee = $this->getEmployeeService()->getEmployee($empNumber);
        $couCode = $employee->getCountry();
        $opeCou = $this->getOperationalCountryService()->getOperationalCountryForCountryCode($couCode);
        $operationalCountry = $opeCou->getFirst();
         if (!is_null($couCode) && !is_null($employee)){
        	$couCode = $employee->getCountry();
        	if (!is_null($couCode)){
        		$opeCou = $this->getOperationalCountryService()->getOperationalCountryForCountryCode($couCode);
        		if (!is_null($opeCou) && !is_null($opeCou->getFirst())){
        			$operationalCountry = $opeCou->getFirst();
        		}
        	}
        }
        ...
        if ($operationalCountry){
        	$isHoliday = $workSchedule->isHoliday($day, $operationalCountry);
        }else{
        	$isHoliday = $workSchedule->isHoliday($day);
        }
//         $isHoliday = $workSchedule->isHoliday($day);// Loc edit code like above
        return $isHoliday;
    }
