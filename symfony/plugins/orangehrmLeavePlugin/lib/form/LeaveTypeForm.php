...
class LeaveTypeForm extends orangehrmForm {
    ...
    /**
     * get Method for OperationalCountry Service
     *
     * @return OperationalCountryService $operationalCountryService
     */
    public function getOperationalCountryService() {
    	if (is_null($this->operationalCountryService)) {
     ...
     * @return array
     */
    private function getCountryListWithoutDefault() {
    	$list = array();
    	$countries = $this->getCountryService()->getCountryList();
    	foreach ($countries as $country) {
    		$list[$country->cou_code] = $country->cou_name;
    ...
    public function configure() {
        ...
        $widgets = array(
            'txtLeaveTypeName' => new sfWidgetFormInput(array(), array('size' => 30)),
        		 ...
        		 * Loc add country
        		 */
//         		'country' => new sfWidgetFormSelect(array('choices' => $this->getCountryList())),
        		/**
        		 * end
        		 */
        ...
        $validators = array(
            ...
            'hdnLeaveTypeId' => new sfValidatorString(array('required' => false)),
        		 ...
        		 * Loc add country
        		 */
//         		'country' => new sfValidatorChoice(array('required' => true, 'choices' => array_keys($this->getCountryListWithoutDefault())))
        		/**
        		 * Loc end
        		 */
        );
        ...
        $requiredLabel = '<em>*</em>';
        $this->getWidgetSchema()->setLabel('txtLeaveTypeName', __('Name') . $requiredLabel);
        $this->getWidgetSchema()->setLabel('excludeIfNoEntitlement', '<a id="exclude_link" href="#">' . __('Is entitlement situational') . '</a>');
		/**
		 * @author Loc
		 */
//         $this->getWidgetSchema()->setLabel('country', __('Working Country') . $requiredLabel);
        /**
         * Loc end
         */
...
//             $this->setDefault('country', $leaveTypeObject->getOperationalCountry()->getCountryCode());
        }
    }
    ...
    public function getLeaveTypeObject() {
		...
		/**
                         * @author Loc
                         * Set operational country for LeaveType
                         */
                        try{
                        	if (!is_null($this->getValue('country'))){
                        		$operationalCountry = $this->getOperationalCountryService()->getOperationalCountryForCountryCode($this->getValue('country'));
                        		if (!is_null($this->getValue('country')) && strlen($this->getValue('country')) > 0){
                        			$leaveType->setOperationalCountry($operationalCountry[0]);
                        		}else{
                        ...
                        /**
                         * End set operational country for LeaveType
                         */
        return $leaveType;
    }
